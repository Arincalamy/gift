...
interface AddCardModalProps {
  isOpen: boolean;
  onClose: () => void;
  onAddCard: (cardData: { name: string; amount: number; hasExpiration: boolean; expiryDate: string | null; usageLimit: number; }) => void;
}
...
const AddCardModal: React.FC<AddCardModalProps> = ({ isOpen, onClose, onAddCard }) => {
  ...
  const handleSubmit = useCallback((e: React.FormEvent) => {
    e.preventDefault();
    if (name.trim() && amount && usageLimit && Number(usageLimit) >= 1) {
      onAddCard({
        name: name.trim(),
  ...
  }, [name, amount, hasExpiration, expiryDate, usageLimit, onAddCard, onClose]);
    ...
    <div className="fixed inset-0 bg-black bg-opacity-70 flex justify-center items-center z-50 transition-opacity" onClick={onClose}>
      <div className="bg-gray-800 rounded-lg shadow-2xl p-8 w-full max-w-md m-4 transform transition-all" onClick={e => e.stopPropagation()}>
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-2xl font-bold text-white">Create New Gift Card</h2>
          <button onClick={onClose} className="text-gray-500 hover:text-white">&times;</button>
        </div>
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label htmlFor="name" className="block text-sm font-medium text-gray-300">Name / Recipient</label>
            <input
              type="text"
              id="name"
              value={name}
           ...
           <div>
            ...
            <div className="mt-1 flex rounded-md shadow-sm">
                <input
                    ...
                    id="usageLimit"
                    value={usageLimit}
                    onChange={(e) => setUsageLimit(e.target.value === '' ? '' : Math.max(1, Number(e.target.value)))}
                    min="1"
                    step="1"
                    className="flex-1 block w-full bg-gray-700 border border-gray-600 rounded-none rounded-l-md py-2 px-3 text-white focus:outline-none focus:ring-indigo-500 focus:border-indigo-500"
                    required
                />
                <button type="button" onClick={() => setUsageLimit(1)} className="inline-flex items-center px-3 rounded-none border border-l-0 border-gray-600 bg-gray-600 text-gray-300 text-sm hover:bg-gray-500">1x</button>
                <button type="button" onClick={() => setUsageLimit(5)} className="inline-flex items-center px-3 rounded-none border border-l-0 border-gray-600 bg-gray-600 text-gray-300 text-sm hover:bg-gray-500">5x</button>
                <button type="button" onClick={() => setUsageLimit(10)} className="inline-flex items-center px-3 rounded-r-md border border-l-0 border-gray-600 bg-gray-600 text-gray-300 text-sm hover:bg-gray-500">10x</button>
            </div>
          </div>
          <div className="flex items-center justify-between bg-gray-700/50 p-3 rounded-md">
            ...
            <label htmlFor="expiration-toggle" className="flex items-center cursor-pointer">
